      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          <div className="flex justify-between items-start">
            <div><p className="text-sm text-gray-500">Order</p><h1 className="text-2xl font-bold">#{String(order.id).slice(0, 8)}</h1></div>
            <span data-testid="order-status" className={`px-4 py-1.5 rounded-full text-sm font-bold ${cancelled ? "bg-red-100 text-red-700" : paid ? "bg-emerald-100 text-emerald-700" : "bg-green-100 text-green-700"}`}>{order.status}</span>
          </div>
        ...
        {!cancelled && (
          <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
            <div className="flex justify-between relative">
              ...
              <div className="absolute top-6 left-6 h-1 bg-gradient-to-r from-red-500 to-orange-500 z-0 transition-all duration-500" style={{ width: `calc(${(Math.max(progress, 0) / (STEPS.length - 1)) * 100}% - 24px)` }} />
              {STEPS.map((s, idx) => {
                const active = idx <= progress;
                return (
                  <div key={s.key} className="relative z-10 flex flex-col items-center">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center text-2xl ${active ? "bg-gradient-to-br from-red-500 to-orange-500 text-white shadow-lg" : "bg-gray-200"}`}>{s.icon}</div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          ...
          <div className="flex justify-between border-t mt-3 pt-3 font-bold text-lg"><span>Total</span><span className="text-red-600">₹{Number(order.grandTotal).toFixed(2)}</span></div>
        </div>
        <div className="text-center"><Link to="/" className="text-red-600 font-semibold hover:underline">← Back to Menu</Link></div>
      </div>
    </div>
  );
}
