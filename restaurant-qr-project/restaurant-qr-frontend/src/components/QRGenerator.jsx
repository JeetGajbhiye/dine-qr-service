...
import { authFetch, PUBLIC_MENU_URL } from "../utils/api";
...
export default function QRGenerator() {
  ...
  const createTable = async (e) => {
    ...
    const res = await authFetch("/tables", {
      method: "POST",
      body: JSON.stringify({ ...form, qrUrl, isActive: true }),
    });
    if (res && res.ok) {
      ...
      alert(j.error || "Failed to create table");
    }
  };
  ...
  const printQR = (table) => {
    ...
    w.document.write(`
      ...
      <style>
        ...
        h1{color:#dc2626;margin:0 0 8px;} p{color:#666;margin-bottom:20px;}
      </style></head><body>
        <div class="card">
          <h1>Scan to Order</h1>
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8 flex-wrap gap-3">
          <div>
            ...
            <p className="text-gray-500 mt-1">Create unique QR codes for each table</p>
          </div>
        ...
        <form onSubmit={createTable} className="bg-white rounded-2xl shadow p-6 mb-8 grid grid-cols-1 md:grid-cols-4 gap-3">
          <input required placeholder="Table Number (e.g. T1)" value={form.tableNumber}
            onChange={(e) => setForm({ ...form, tableNumber: e.target.value })}
            className="border rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-red-400 outline-none" />
          <input placeholder="Label (optional)" value={form.label}
            onChange={(e) => setForm({ ...form, label: e.target.value })}
            className="border rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-red-400 outline-none md:col-span-2" />
          <button className="bg-red-600 hover:bg-red-700 text-white font-semibold rounded-xl py-2.5">+ Add Table</button>
        </form>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-white rounded-2xl shadow">
            ...
            <p className="text-gray-500">No tables yet. Add one to generate its QR code.</p>
          </div>
        ) : (
            ...
            {tables.map((t) => (
              <div key={t.id} className="bg-white rounded-2xl shadow p-6 flex flex-col items-center">
                <div className="w-full flex justify-between items-start mb-3">
                  <div>
                    <h3 className="font-bold text-lg">Table {t.tableNumber}</h3>
                ...
                <div className="p-3 bg-white border-2 border-dashed border-gray-200 rounded-xl">
                  <QRCodeCanvas id={`qr-${t.tableNumber}`}
                    value={t.qrUrl || `${PUBLIC_MENU_URL}/?table=${t.tableNumber}`}
                    size={200} level="H" includeMargin />
                </div>
                <p className="text-xs text-gray-400 mt-3 break-all text-center">{t.qrUrl}</p>
                <div className="flex gap-2 mt-4 w-full">
                  <button onClick={() => downloadQR(t.tableNumber)}
                    className="flex-1 bg-gray-900 text-white rounded-lg py-2 text-sm font-semibold hover:bg-gray-700">⬇ Download</button>
                  <button onClick={() => printQR(t)}
                    className="flex-1 bg-red-600 text-white rounded-lg py-2 text-sm font-semibold hover:bg-red-700">🖨 Print</button>
                </div>
              </div>
            ))}
          </div>
