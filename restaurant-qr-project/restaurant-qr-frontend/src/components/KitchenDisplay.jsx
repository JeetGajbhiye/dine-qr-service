        ...
        <div className="max-w-full px-6 py-3 flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Live queue • refreshes every {REFRESH_MS / 1000}s</p>
            </div>
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            <button onClick={() => setSoundOn((s) => !s)}
              className={`px-3 py-1.5 rounded-full text-sm font-semibold ${soundOn ? "bg-green-500" : "bg-gray-600"}`}>
                ...
                <div key={o.id} className={`bg-white/10 rounded-xl p-4 ${urgencyRing(o.createdAt)}`}>
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      {o.tableNumber && <span className="text-xs bg-red-500/80 rounded-full px-2 py-0.5">Table {o.tableNumber}</span>}
                    </div>
                    <div className="text-right text-xs text-gray-300">
                      <div>{timeAgo(o.createdAt)}</div>
                      <div className="text-gray-500">{new Date(o.createdAt).toLocaleTimeString()}</div>
                    </div>
                  </div>
                  <ul className="space-y-1 mb-3">
                    {(o.items || []).map((i) => (
                      <li key={i.id} className="flex justify-between text-sm bg-black/20 rounded-lg px-2.5 py-1.5">
                        <span className="font-semibold">{i.name}</span>
                        <span className="text-orange-300 font-bold">×{i.quantity}</span>
                      </li>
                    ))}
                  </ul>
                  {o.customerName && <p className="text-xs text-gray-400">👤 {o.customerName}</p>}
                  <button onClick={() => advance(o, col.next)}
                    className={`w-full mt-3 bg-gradient-to-r ${col.color} font-bold py-2 rounded-lg hover:opacity-90`}>
