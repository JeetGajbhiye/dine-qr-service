...
import { API_BASE, WHATSAPP_NUMBER, RAZORPAY_KEY, publicFetch } from "../utils/api";

const CATEGORIES = ["All", "Starters", "Main Course", "Beverages", "Desserts"];

const FALLBACK_MENU = [
  { id: 1, name: "Paneer Tikka", description: "Char-grilled cottage cheese in yogurt & spices.", price: 249, imageUrl: "https://images.unsplash.com/photo-1567188040759-fb8a883dc6d8?w=800", category: "Starters", isVeg: true },
  { id: 2, name: "Butter Chicken", description: "Creamy tomato gravy with tender chicken.", price: 349, imageUrl: "https://images.unsplash.com/photo-1603894584373-5ac82b2ae398?w=800", category: "Main Course", isVeg: false },
  { id: 3, name: "Masala Chai", description: "Indian spiced milk tea.", price: 49, imageUrl: "https://images.unsplash.com/photo-1571934811356-5cc061b6821f?w=800", category: "Beverages", isVeg: true },
  { id: 4, name: "Gulab Jamun", description: "Warm dumplings soaked in cardamom syrup.", price: 99, imageUrl: "https://images.unsplash.com/photo-1601050690597-df0568f70950?w=800", category: "Desserts", isVeg: true },
];
...
export default function MenuApp() {
  ...
  // Auto-fill table from QR ?table=T1
  useEffect(() => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await publicFetch("/menu");
        if (!res.ok) throw new Error();
        const data = await res.json();
        setMenu(data.length ? data : FALLBACK_MENU);
      } catch {
  ...
  const grandTotal = useMemo(
    () => cartItems.reduce((s, ci) => s + ci.item.price * ci.qty, 0),
    [cartItems]
  );
  ...
  const buildWhatsAppMessage = (orderId) => {
    ...
    cartItems.forEach((ci, idx) => {
      const lineTotal = (ci.item.price * ci.qty).toFixed(2);
      lines.push(`${idx + 1}. ${ci.item.name}  x${ci.qty}  -  ₹${lineTotal}`);
    });
    lines.push(`--------------------------------`);
  ...
  // Persist order in backend before opening WhatsApp
  ...
  const checkoutViaWhatsApp = async () => {
    ...
    try {
      ...
      const msg = buildWhatsAppMessage(created.id);
  ...
  const checkoutRazorpay = () => {
    ...
    const options = {
      ...
      description: `Order for ${totalQty} item(s)`,
      image: "/favicon.svg",
      handler: async function (response) {
        try {
          ...
          alert("Payment successful but order save failed. Please contact staff.");
        }
      },
      prefill: {
        name: customer.name,
      ...
      notes: { table: customer.table },
      theme: { color: "#dc2626" },
      modal: { ondismiss: () => console.log("Razorpay closed") },
    };
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-red-50">
      ...
      <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-md shadow-sm border-b border-orange-100">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-red-500 to-orange-500 flex items-center justify-center text-white text-xl font-bold shadow-md">
              🍽️
            </div>
            <div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {totalQty > 0 && (
                <span className="absolute -top-2 -right-2 bg-white text-red-600 text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center shadow">
                  {totalQty}
                </span>
              )}
            </button>
      ...
      <section className="max-w-7xl mx-auto px-4 py-6 md:py-10">
        <div className="rounded-2xl bg-gradient-to-r from-red-500 via-orange-500 to-amber-500 text-white p-6 md:p-10 shadow-lg">
          ...
          {customer.table && (
            <div className="mt-4 inline-block bg-white/20 backdrop-blur px-4 py-2 rounded-full text-sm font-semibold">
            ...
            {filteredMenu.map((item) => (
              ...
              >
                <div className="relative h-48 overflow-hidden">
                  ...
                  />
                  <span className="absolute top-3 left-3 bg-white/95 text-gray-800 text-xs font-semibold px-2.5 py-1 rounded-full shadow">
                    {t(`categories.${item.category}`)}
                  </span>
                  {typeof item.isVeg === "boolean" && (
                    <span className={`absolute top-3 right-3 w-6 h-6 rounded border-2 flex items-center justify-center bg-white ${
          ...
          <aside className="relative w-full sm:max-w-md bg-white h-full shadow-2xl flex flex-col animate-slide-in">
              ...
              {cartItems.length > 0 && (
                <div className="pt-2 space-y-3">
                  ...
                  <input type="tel" placeholder={t("phone")} value={customer.phone}
                    ...
                    className="w-full border rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-red-400" />
                  <input type="email" placeholder={t("email")} value={customer.email}
                    onChange={(e) => setCustomer({ ...customer, email: e.target.value })}
                    className="w-full border rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-red-400" />
                  <input type="text" placeholder={t("table")} value={customer.table}
            ...
            {cartItems.length > 0 && (
              <div className="border-t p-5 bg-gray-50 space-y-3">
                ...
                <button onClick={checkoutViaWhatsApp} disabled={placing}
                ...
                <button onClick={checkoutRazorpay} disabled={placing}
